...
| 文档版本 | v1.3.3                                                                                                                                                                                            |
| 变更说明 | v1.3.2 及以前见历史版本。v1.3.3：**§10.7** 仅保留高风险模态 **S1、S3**（删除不存在的清数据 S2、删除报告重测 S4）；**§6.3** 首页顺序与完成页「下一项」**一致**；**§6.5 / §11.8** 主链路完成页空闲 **20 秒**（首次点击后倒计时）；**§11.2** 量产区分文案；**§11.6/11.7** 精简；验收与自检同步。 |
...
| 指定设备覆盖      | 仅绑定到**所选设备序列号（SN）的一份配置，表示该台的独立策略**。产品沟通上按「该 SN 的整包有效配置」理解；**不是**对门店默认作「增量 diff」的叙述，以免运营误解「只改了一个开关」却实际落库整套快照。若量产采用真正的 diff/补丁模型，由研发在接口契约或附录中单独说明；当前静态 Demo 为按 SN **存完整覆盖副本**（与测量配置 Demo 中设备覆盖结构一致）。 |
...
| 清除指定覆盖（若后台提供）  | 清除后该 SN **回退**为门店默认；建议后台对「清除覆盖」使用**二次确认**，避免误触；设备端仅消费下发后的结果。                                                              |
