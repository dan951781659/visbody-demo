...
| 文档版本 | v1.3.4 |
| 变更说明 | v1.3.3 及以前见历史版本。v1.3.4：**§7.7.2** 明确量产下后台配置在设备侧**须重启后生效**（非运行中即时切换），并要求 WellnessHub **固定提示文案**；删除「清除覆盖」二次确认建议。 |
...
3. **测量准备清单**：展示扎发、赤脚、空腹、无运动四项；用户通过手势或点击确认。若门店关闭了「显示测量准备页」，则本步跳过，直接进入转台站姿。
4. **转台站姿**：用户站上转台并保持静止；设备识别站姿。因身高测量默认关闭，此处仅出现**体重**相关的简短提示（如体重测量已完成），数值进入后台用于体成分计算，**不在流程页展示体重读数**。用户按提示点击「下一步」进入扶手准备（Demo 默认需手动点击换页）。
5. **扶手准备**：用户双手握紧扶手，完成金属接触识别。
6. **双臂约 45°**：用户按图示摆好姿势，完成姿态识别。
...
- **演示文件**：WellnessHub 测量配置 Demo 单页（见依据中的 html 文件名）。
...
**报告显示（WellnessHub）**：右侧「报告显示配置」按**当前已开启的测量项目**聚合列出各组报告开关，随左侧项目开关即时增减，**不要求**先点选某一项目再出现报告区。

**存储与同步规则**

- **一般规则**：综合测量与「身体成分 / 体围」等单项的报告配置**分桶存储**；综合与身体成分、体围单项因互斥不会同时生效，切换后各自保留一套配置。
- **B1 例外（综合 + 体态单项同时开启）**：体态报告、脊柱报告、臀型报告共 **3 个键**（`postureReport`、`spine`、`hip`）在 `projects.comprehensive.reports` 与 `projects.posture.reports` 中**必须保持一致**。后台在加载、保存、切换项目开关或拨动上述任一开关时，以 **综合测量内体态分组** 为权威源，将三者同步写入体态单项存储；设备端 `reportVisibility` 对该三键只合并一次，避免后写覆盖导致与综合不一致。
...
WellnessHub 对同一门店存在**两层**配置来源：**门店默认**与**指定设备覆盖**。以下说明二者关系、编辑交互与设备侧生效方式（与 §7.8 衔接）。

#### 7.7.1 概念模型

| 类型 | 含义 |
|------|------|
| 门店默认 | 该门店的**基线配置**；对该门店下**未配置或未命中**设备覆盖的终端生效。 |
| 指定设备覆盖 | 仅绑定到**所选设备序列号（SN）**的一份配置，表示该台的**独立策略**。产品沟通上按「该 SN 的整包有效配置」理解；**不是**对门店默认作「增量 diff」的叙述，以免运营误解「只改了一个开关」却实际落库整套快照。若量产采用真正的 diff/补丁模型，由研发在接口契约或附录中单独说明；当前静态 Demo 为按 SN **存完整覆盖副本**（与测量配置 Demo 中设备覆盖结构一致）。 |
| 编辑不反向改写门店默认 | 在「指定设备覆盖」下保存时，**只**更新所选 SN（或多台同时编辑时**各自**）的覆盖内容，**不**回写门店默认。 |

#### 7.7.2 交互与生效规则

| 规则 | 产品含义 |
|------|----------|
| 设备最终读哪一套 | 该 SN **存在**有效覆盖记录 → 设备采用**覆盖**；否则 → 采用**门店默认**。解析结果即本条与 §7.8 中的「WellnessHub 下发的测量项目与报告配置」在业务上的**有效快照**（先解析门店/覆盖，再与设备侧合并）。 |
| 修改门店默认 | **不会自动**删除、合并或改写已有设备覆盖条目；已覆盖设备在运营**再次保存该 SN 覆盖**或执行**清除覆盖**（若后台提供）之前，**继续**使用原覆盖内容。 |
| 新设备 / 从未覆盖的 SN | 无覆盖记录时，**直接**采用门店默认。 |
| 清除指定覆盖（若后台提供） | 清除后该 SN **回退**为门店默认；设备端仅消费下发后的结果。 |
| 后台保存后在设备侧的生效时机（量产） | 后台保存并成功**下发**（入云、设备可按 SN 拉取到新版本）后，设备端**须重启设备**（含整机重新上电后首次完成配置拉取等等价路径）后，测量项目/报告/首页顺序等 WellnessHub 配置方视为**完整生效**；**非**「后台保存瞬间、设备正在运行中立即切换」。浏览器同源 Demo 刷新即读本地快照，**仅用于联调**，不代表量产即时生效。 |
| WellnessHub 提示文案（必带） | 测量配置保存成功界面或配置编辑区须向运营展示**固定说明**，语义须包含：**保存后请重启对应体测设备，配置在设备重启后生效**（具体措辞由 UX/文案统一，不可省略「重启」与「生效条件」）。 |
| 多选设备（后台） | **预览**通常以第一台为准；**保存**覆盖时对勾选列表逐台写入（与 Demo「多选设备同步」演示一致）。量产以接口与权限设计为准；量产需每台设备按自身序列号拉取配置。 |
...
flowchart TD
  ...
  check -->|否| useStore[采用门店默认]
  check -->|是| useOv[采用指定设备覆盖]
...
后台 Demo 在保存或刷新预览时，将一份配置快照写入浏览器存储；设备端 Demo 从该快照读取。同源部署（同一站点）才能联调。具体存储键名见附录「研发对照表」。测量配置 Demo 顶部的**生效方式**（门店默认 / 指定设备覆盖）与设备列表勾选行为，用于演示 §7.7 的交互关系，生产目标为「后台按 SN 下发与版本化管理」下的等价体验。

**与量产的差异**：Demo 无真实设备进程；**刷新设备端演示页**即可读到最新存储，**不**模拟「须物理重启整机」。量产侧生效规则见 **§7.7.2「后台保存后在设备侧的生效时机」**；后台产品文案仍按 §7.7.2 的「必带」提示展示，避免运营误以为保存即现场生效。
...
- [ ] 量产：后台保存并成功下发后，设备端新配置在**重启设备**后完整生效（见 §7.7.2）；WellnessHub 在保存成功等节点展示 **§7.7.2** 要求的固定提示文案。
...
| 后台配置在设备侧生效 | 保存后**刷新**设备端演示页即读新快照 | 保存并成功下发后，设备**须重启**方完整生效（见 §7.7.2）；后台须展示重启提示 |
...
**设备侧采用新下发的完整配置**：按 **§7.7.2** 量产规则，须在设备**重启后**生效；WellnessHub 须在保存成功等关键节点展示 **§7.7.2** 所列固定提示文案，避免运营误解。
