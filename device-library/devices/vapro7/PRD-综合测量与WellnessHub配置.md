...
- 转台识别通过后 **自动**完成流程内身高/体重采集并进入扶手环节（与 §5.3 一致）；量产与 Demo 主链路一致。其余 HTML 页在讲解场景下仍可手动换页；部分页支持 `?demoAuto=1` 等调试参数（见 [`LOCAL-DEMO.md`](./demo/LOCAL-DEMO.md)）。
...
- **综合主链路**（准备清单 → 转台 → 扶手 → 45°）在 Demo 中已实现转台自动采高体重、扶手语音后自动进 45° 等自动推进；**其余页面**默认不自动跳转下一 HTML，便于产品讲解。设备能力「语音播报」关闭时，仍应保留触控完成路径。
