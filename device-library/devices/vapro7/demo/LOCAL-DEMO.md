...
2. 同标签页或新标签打开 `home.html` / `report-scan-login.html` / `report-detail.html`，**刷新**即可看到入口与完成测量支路、报告卡片随配置变化。Hub 改配置后设备页须刷新（同源 `localStorage`）；量产须重启设备（见综合 PRD §7.7.2）。

## URL 参数（调试）

设备页 URL 查询参数可覆盖部分流程态与 Hub 配置（如 `runtimeMode`、`comprehensive` 等），便于分享单页演示；**WellnessHub 已保存的 `vapro7-measurement-config` 在 `loadState` 时优先合并**。用户已在设备上改过的本地能力键（`vapro7-demo-state`）以 localStorage 为准。

## 与 PRD 的 Demo 分期说明

Demo **v1.9.5** 已实现 PRD §6.2 核心规则：环形「继续下一项」、体重不进循环、末项仅「完成测量」、全项目统一 `standard-next-step.html` 完成页。量产重启生效等仍以 PRD 为准。
