...
const DEMO_VERSION_FALLBACK = "1.3.1";

const NEXT_RECOMMEND_DESC = {
  standard: "一圈完成基础体态采集",
  pro: "高精度体态评估",
  singleShoulder: "肩部专项测量",
  singleBalance: "平衡专项测量",
  singleNeck: "颈部专项测量"
};
...
function speakCountdownDigit(seconds, voiceEnabled) {
...
function setupStageAdvance() {
  document.querySelectorAll("[data-stage-next]").forEach((el) => {
    ...
    function startStage() {
      ...
      hasStarted = true;
      const voiceOn = loadState().voiceEnabled;
      if (secondsEl) {
        secondsEl.textContent = String(remainingSeconds);
      }
      speakCountdownDigit(remainingSeconds, voiceOn);
      if (delay <= 0) {
        finishStage();
        return;
      }
      timer = window.setInterval(() => {
        ...
        if (remainingSeconds > 0) {
          speakCountdownDigit(remainingSeconds, loadState().voiceEnabled);
        }
        if (remainingSeconds <= 0) {
          finishStage();
        }
...
function setupGeneratingFlow() {
  ...
  if (state.voiceEnabled) speakText(voice);
...
function setupMeasuringTurntable() {
  ...
  function playTickBurst() {
    if (!motorCtx || reducedMotion || !state.soundEffectsEnabled) return;
    try {
  ...
  if (footer) {
    footer.classList.add("measuring-footer--locked");
  ...
  const tick = () => {
    ...
    if (remaining <= 10000 && remaining > 0 && focusEl) {
    ...
    if (motorGain && motorCtx) {
      try {
        motorGain.gain.linearRampToValueAtTime(0.012, motorCtx.currentTime + 0.3);
      } catch {
        /* noop */
      }
    }
    ...
    if (focusEl) focusEl.textContent = "请不要移动";
    if (chipEl) chipEl.textContent = "转台已停";
    if (footer) {
      footer.classList.remove("measuring-footer--locked");
  ...
  function finishMeasuring() {
    if (!measuringDone) return;
    stopSound();
    navigateToTarget(withStateQuery(nextTarget, loadState()));
  }

  footer.addEventListener("click", finishMeasuring);
